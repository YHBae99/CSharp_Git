...
namespace _08_CodeOrganization
{
    public partial class Form1 : Form
    {
        ...
        private string fReplaceResult(string strBeforeText, int idgRow)
        {
            ...
            // DataGridView에서 변환 전/후 값을 가져옴
            string beforeValue = dgChangedate["cBefore", idgRow].Value.ToString();
            string afterValue = dgChangedate["cAfter", idgRow].Value.ToString();

            // 입력 텍스트에서 변환 전 값을 변환 후 값으로 대체
            string strAfterText = strBeforeText.Replace(beforeValue, afterValue);
